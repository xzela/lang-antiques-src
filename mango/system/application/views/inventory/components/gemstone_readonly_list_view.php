...
<table id="gemstone_table">
	...
	<tr>
		...
		<td class='stone_title' colspan='8'></td>
	</tr>
    <?php if(sizeof($diamonds) > 0 ): ?>
        <?php foreach($diamonds as $dia): ?>
            <?php $total_diamond_quantity += $dia['d_quantity']; ?>
            <?php $total_diamond_weight += $dia['d_carats']; ?>
            <tr>
                <td><?php echo $gemstone_names[$dia['d_type_id']]['stone_name'] ?></td>
    ...
    <?php if(sizeof($gemstones) > 0): ?>
        	...
        	<?php foreach($gemstones as $gem): ?>
        		<tr>
        			<td><?php echo $gemstone_names[$gem['gem_type_id']]['stone_name']; ?></td>
    ...
    <?php if(sizeof($pearls) > 0): ?>
    	...
    	<?php foreach($pearls as $pearl): ?>
    		<tr>
    			<td><?php echo $gemstone_names[$pearl['p_type_id']]['stone_name']; ?></td>
    ...
    <?php if(sizeof($jadeite) > 0): ?>
    	...
    	<?php foreach($jadeite as $jade): ?>
    		<tr>
    			<td><?php echo $gemstone_names[$jade['j_type_id']]['stone_name']; ?></td>
    ...
    <?php if(sizeof($opals) > 0): ?>
    	...
    	<?php foreach($opals as $opal): ?>
    		<tr>
    			<td><?php echo $gemstone_names[$opal['o_type_id']]['stone_name']; ?></td>
